 ...
 * 创建登录窗口
 */
async function createLoginWindow(context) {
    const window = new electron_1.BrowserWindow({
        width: config_1.LOGIN_WINDOW_WIDTH,
        height: config_1.LOGIN_WINDOW_HEIGHT,
        resizable: false,
    ...
    // 加载登录页
    if (config_1.isDev) {
        const serverReady = await (0, utils_1.waitForServer)(config_1.PAGE_URLS.LOGIN);
        if (serverReady) {
            await new Promise((resolve) => setTimeout(resolve, 500));
            window.loadURL(config_1.PAGE_URLS.LOGIN);
        }
        else {
